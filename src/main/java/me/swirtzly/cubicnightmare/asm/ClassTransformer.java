package me.swirtzly.cubicnightmare.asm;
			...
			for (int i = 0; i < updateLightmap.instructions.size(); i++) {
				...
				if (an instanceof LdcInsnNode) {
					...
					if (lin.cst.equals(m0) || lin.cst.equals(m3)) {
						updateLightmap.instructions.insert(lin, new MethodInsnNode(Opcodes.INVOKESTATIC, "me/swirtzly/cubicnightmare/asm/ASMHooks", "up", "(F)F", false));
					} else if (lin.cst.equals(a0) || lin.cst.equals(a3)) {
						updateLightmap.instructions.insert(lin, new MethodInsnNode(Opcodes.INVOKESTATIC, "me/swirtzly/cubicnightmare/asm/ASMHooks", "down", "(F)F", false));
					}
					...
					if (fin.name.equals(CNPlugin.IN_MCP ? "gammaSetting" : "field_74333_Y")) {
						updateLightmap.instructions.insert(fin, new MethodInsnNode(Opcodes.INVOKESTATIC, "me/swirtzly/cubicnightmare/asm/ASMHooks", "overrideGamma", "(F)F", false));
					}
					...
					if (min.name.equals(CNPlugin.IN_MCP ? "updateDynamicTexture" : "func_110564_a")) {
						...
						toInsert.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "me/swirtzly/cubicnightmare/asm/ASMHooks", "modifyLightmap", "([I)[I", false));
